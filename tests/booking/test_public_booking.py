...
class TestBooking:
    ...
    def test_create_booking(self, booking_client: PublicBookingClient):
        """
        ...
        assert_create_booking_response(request, response_data)
    ...
    def test_create_booking_short_firstname(self, booking_client: PublicBookingClient):
        ...
        """
        request = CreateBookingRequestFactory.build()
        request.firstname = "Ab"  # Length 2 (< 3)
    ...
    @allure.severity(Severity.NORMAL)
    def test_create_booking_high_level(self, booking_client: PublicBookingClient):
        """
        Positive test: Use convenience method create_booking().
        Returns parsed Pydantic model CreateBookingResponseSchema.
        """
        request = CreateBookingRequestFactory.build()
        response_data = booking_client.create_booking(request)
    ...
    @allure.story(AllureStory.AVAILABILITY_CHECK)
    @allure.tag(AllureTag.GET_ENTITIES)
    @allure.title("GET /booking/unavailable - Check availability during booked period")
    @allure.severity(Severity.NORMAL)
    ...
    @allure.title("GET /booking/unavailable - Missing {missing_param} parameter (400)")
    ...
    @allure.story(AllureStory.AVAILABILITY_CHECK)
    @allure.tag(AllureTag.GET_ENTITIES, AllureTag.NEGATIVE)
    @allure.title("GET /booking/unavailable - Invalid date (non-existent day) (500)")
    ...
    @allure.story(AllureStory.BOOKING_SUMMARY)
    @allure.tag(AllureTag.GET_ENTITIES)
    @allure.title("GET /booking/summary - Get summary with non-positive roomid (empty list)")
    ...
    @allure.story(AllureStory.BOOKING_SUMMARY)
    @allure.tag(AllureTag.GET_ENTITIES, AllureTag.NEGATIVE)
    @allure.title("GET /booking/summary - Invalid roomid (non-number) (500)")
